"""
Training Script with Embedding Tracking

This script trains the transformer on 10,000 examples and saves embedding vectors
evolution to embeddings.txt throughout training.
"""
...
def save_embeddings(model, epoch, filepath='embeddings.txt', mode='a'):
    """
    ...
    with open(filepath, mode) as f:
        ...
        if epoch == -1:
            f.write("=" * 80 + "\n")
            f.write("INITIAL EMBEDDINGS (Before Training)\n")
            f.write("=" * 80 + "\n")
        else:
            f.write("\n" + "=" * 80 + "\n")
...
def save_embedding_statistics(model, epoch, filepath='embeddings.txt'):
    """
    Save statistical summary of embeddings.
    ...
    Args:
        ...
        filepath: Output file path
    """
    embeddings = model.embedding.embeddings

    with open(filepath, 'a') as f:
        f.write("-" * 80 + "\n")
        f.write(f"Statistics (Epoch {epoch + 1}):\n")
        f.write("-" * 80 + "\n")

        # Overall statistics
        f.write(f"  Overall Mean:     {np.mean(embeddings):10.5f}\n")
        f.write(f"  Overall Std Dev:  {np.std(embeddings):10.5f}\n")
        f.write(f"  Overall Min:      {np.min(embeddings):10.5f}\n")
        f.write(f"  Overall Max:      {np.max(embeddings):10.5f}\n")
        f.write("\n")

        # Per-token norms
        f.write("  Token Embedding Norms (L2):\n")
        for token_idx in range(VOCAB_SIZE):
            token_name = REVERSE_VOCAB[token_idx]
            norm = np.linalg.norm(embeddings[token_idx])
            f.write(f"    {token_name:8s}: {norm:8.5f}\n")

        f.write("\n")

        # Pairwise similarities for operations
        operation_tokens = ['First', 'Second', 'Last', 'Max', 'Min']
        operation_indices = [VOCAB[op] for op in operation_tokens]

        f.write("  Operation Embedding Similarities (Cosine):\n")
        for i, op1 in enumerate(operation_tokens):
            idx1 = operation_indices[i]
            emb1 = embeddings[idx1]
            norm1 = np.linalg.norm(emb1)

            for j, op2 in enumerate(operation_tokens):
                if j <= i:
                    continue
                idx2 = operation_indices[j]
                emb2 = embeddings[idx2]
                norm2 = np.linalg.norm(emb2)

                # Cosine similarity
                similarity = np.dot(emb1, emb2) / (norm1 * norm2 + 1e-8)
                f.write(f"    {op1:8s} <-> {op2:8s}: {similarity:8.5f}\n")

        f.write("\n")


def train_and_track_embeddings():
    """Main training function with embedding tracking."""

    print("=" * 80)
    print("TRANSFORMER TRAINING WITH EMBEDDING TRACKING")
    print("=" * 80)
    print()
    ...
    # =========================================================================
    print("Saving initial embeddings to embeddings.txt...")
    save_embeddings(model, epoch=-1, filepath='embeddings.txt', mode='w')
    save_embedding_statistics(model, epoch=-1, filepath='embeddings.txt')
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == num_epochs - 1:
            ...
            save_embeddings(model, epoch, filepath='embeddings.txt', mode='a')
            save_embedding_statistics(model, epoch, filepath='embeddings.txt')
    ...
    save_embeddings(model, epoch=num_epochs-1, filepath='embeddings.txt', mode='a')
    save_embedding_statistics(model, epoch=num_epochs-1, filepath='embeddings.txt')
    print("✓ Final embeddings saved")
    print()

    print("=" * 80)
    print("Embeddings saved to: embeddings.txt")
    print("Model checkpoint saved to: checkpoints/best_model.pkl")
    print("=" * 80)


if __name__ == "__main__":
    train_and_track_embeddings()
